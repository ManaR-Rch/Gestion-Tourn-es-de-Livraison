...
public class OpenApiConfig {
  ...
  public OpenApiCustomiser manualPaths() {
    return openApi -> {
      ...
      RequestBody rb = new RequestBody()
          ...
          .content(
              new Content().addMediaType("application/json", new MediaType().schema(new Schema<>().type("object"))));
      ...
      RequestBody postBody = new RequestBody()
          .description(
              "Delivery JSON: { \"latitude\": 48.8, \"longitude\": 2.3, \"weight\": 1.0, \"volume\": 0.1, \"status\": \"PENDING\" }")
          .content(
              new Content().addMediaType("application/json", new MediaType().schema(new Schema<>().type("object"))));
  ...
  /**
   * Ensures any OpenApiCustomiser beans (including manualPaths) are applied to the
   * OpenAPI instance when the context is refreshed. This forces the manual path
   * entries to be present even if no annotated controllers are discovered.
   */
  @Bean
  public ApplicationListener<ContextRefreshedEvent> openApiInitializer(OpenAPI openAPI,
      List<OpenApiCustomiser> customisers) {
    return event -> {
      if (customisers != null) {
        for (OpenApiCustomiser c : customisers) {
          try {
            c.customise(openAPI);
          } catch (Exception ex) {
            System.err.println("OpenApiCustomiser failed: " + ex.getMessage());
          }
        }
      }
    };
